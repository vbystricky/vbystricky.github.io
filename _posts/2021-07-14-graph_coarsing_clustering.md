...
$$E\left(cut(\mathcal A, \mathcal B)\right) = p \cdot \alpha N \cdot (1 - \alpha) N = p \alpha (1 - \alpha) N^2$$

Eсли же мы сделаем разрез, отделив только одну вершину от графа, то математическое ожидание величины такого разреза будет:

$$E\left(cut(v, \mathcal V \setminus \{v\})\right) = p (N - 1)$$
...
$$D(b) = \sum_{v\in \mathcal A} w_{b\,v} - \sum_{v\in \mathcal B} w_{b\,v}.$$
